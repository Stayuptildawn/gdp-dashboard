...
# IMPORTANT: page config
st.set_page_config(
    page_title="UPM Innovation Platform - Edit Idea",
    page_icon="💡",
    layout="wide",
...
# Fill in the form with the idea we're editing
...
with c1:
    # Build the category dropdown from existing data plus fallback options
    options = []
    try:
        options = sorted(
            set(
    ...
    if st.session_state.get("edit_validate") and not (title or "").strip():
        title_warn.markdown(
            f'<div class="warning-message">{st.session_state.get("edit_warning", "Please fill in all required fields")}</div>',
...
estimated_impact = st.text_input(
    "Estimated Impact / Target Audience *",
    value=data["Estimated Impact / Target Audience"],
...
with c2:
    ...
    if terms:
        ...
        terms_warn.markdown(
            '<div class="error-message">You need to accept the terms and conditions.</div>',
            unsafe_allow_html=True,
        )
    ...
    st.info(
        "💡 Once submitted, your idea will be visible to everyone. "
        "Don't worry - you can edit it anytime from 'My Ideas'."
    )

# Cancel button at the bottom – FIXED: no invalid width
if st.button("← Cancel"):
    st.session_state.pop("edit_id", None)
    st.switch_page("pages/myideas.py")
